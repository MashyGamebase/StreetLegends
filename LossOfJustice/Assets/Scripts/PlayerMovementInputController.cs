...
public class PlayerMovementInputController : MonoBehaviour
{
    ...
    private void Update()
    {
        ...
        if(sprintValue == 1f)
        {
            Vector3 sprintPosition = (transform.forward * _move.y * (moveSpeed * 2) + (transform.right * _move.x * (moveSpeed * 2)));
            nextPosition = transform.position + sprintPosition;
        }
        else if(sprintValue != 1f)
        {
